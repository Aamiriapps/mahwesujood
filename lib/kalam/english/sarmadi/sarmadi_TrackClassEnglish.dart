...
class SarmadiTrackclassenglish extends StatefulWidget {
  final String title;
  final String nazam;
  ...
  const SarmadiTrackclassenglish({
    Key? key,
    required this.title,
    required this.nazam,
  ...
  @override
  State<SarmadiTrackclassenglish> createState() => _TrackPlayerScreenNewState();
}

class _TrackPlayerScreenNewState extends State<SarmadiTrackclassenglish> {
          ...
          children: [
            ...
            if (stanzas.isNotEmpty)
              ...stanzas.map((raw) {
                ...
                final stanza = raw.replaceAll('\r\n', '\n').trim();
                final nl = stanza.indexOf('\n');

                final firstLine = nl == -1
                    ? stanza
                    : stanza.substring(0, nl).trim();
                final rest = nl == -1 ? '' : stanza.substring(nl + 1).trim();
                ...
                return Container(
                  ...
                  // One paragraph with mixed styles
                  child: FittedBox(
                    ...
                    child: Text.rich(
                      TextSpan(
                        style: const TextStyle(
                          fontFamily: 'Trajan',
                          color: Colors.white,
                          // base style (sizes overridden below)
                        ),
                        children: [
                          // First line (big)
                          TextSpan(
